...
import { convertCountryCodeToAlpha3 } from '@/lib/country-code-converter'
import { getCountryCodeForProvider } from '@/lib/provider-mapper'
  ...
  try {
    ...
    try {
      ...
      if (isCardPayment) {
        ...
        // Construct proper payment URL using order number
        const orderNumber = checkoutData.source_number
        const paymentUrl = `https://www.dollrpay.link/pay/orders/${orderNumber}?withSummary=false&paymentMethods=CREDIT_CARD`
        ...
        // Only if Firebase is configured (skip in environments without credentials)
        if (isFirebaseConfigured()) {
          const donorId = `${input.email}-${sourceId}`
          saveDonationToFirestore(
            ...
            console.warn('Firestore save error (non-fatal):', error)
          })
        }
      ...
      // Step 3: Create payment account (mobile money only, cards redirect to Dollr)
      let paymentAccountId: string
      // Mobile money
      const phoneFormatted = formatPhoneForDollr(input.mobilePhone!)
      // Use provider's country (e.g., ORANGE_MONEY_LBR -> LR) instead of form's country
      const providerCountryCode = getCountryCodeForProvider(input.detectedProvider) || input.country
      const mobilePayload = {
        ...
        country_code: convertCountryCodeToAlpha3(providerCountryCode),
        insensitive_account_number: phoneFormatted,
      }
      console.log('[Donations] Mobile account payload:', JSON.stringify(mobilePayload, null, 2))
      ...
      if (!mobileResponse.ok) {
        const mobileErrorData = await mobileResponse.text()
        console.error('[Donations] Mobile account create error response:', mobileErrorData)
        throw new Error(`Mobile account create failed: ${mobileResponse.status} - ${mobileErrorData}`)
      }
