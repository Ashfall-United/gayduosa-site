...
/**
 * Get country code for a provider (e.g., MTN_MOMO_LBR -> LR)
 */
export function getCountryCodeForProvider(providerCode: string | undefined): string | null {
  if (!providerCode) return null

  // Extract country code from provider code (e.g., MTN_MOMO_LBR -> LBR)
  const match = providerCode.match(/_([A-Z]{3})$/)
  if (!match) return null

  const alpha3Code = match[1]

  // Map common alpha-3 to alpha-2 codes
  const ALPHA3_TO_ALPHA2: Record<string, string> = {
    'LBR': 'LR',
    'RWA': 'RW',
  }

  return ALPHA3_TO_ALPHA2[alpha3Code] || null
}
